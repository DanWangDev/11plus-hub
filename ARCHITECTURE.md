...
- **CD** (`.github/workflows/deploy.yml`): deploy-on-CI-success via the **org-shared** NAS self-hosted runner, gated by the `ENABLE_NAS_DEPLOY` GitHub variable (set it once the runner is registered). The deploy pulls images, restarts the stack, and waits for `docker exec hub-backend wget http://127.0.0.1:3009/api/health` to report healthy. Runner definition + setup guide live in the [labf-infra](https://github.com/DanWangDev/labf-infra) repo (`nas-runner/`, `docs/nas-runner-setup.md`).
